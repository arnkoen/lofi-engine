	...
	for (int ii = 0; ii < nplaying; ++ii) {
		...
		if (source->flags & TM_SOURCEFLAG_POSITIONAL) {
			...
			if (dist > 1.0e-8f) {
				...
				// Direction from listener to source (normalized)
				const float to_source[3] = {
					(source->position[0] - tm.position[0]) / dist,
					(source->position[1] - tm.position[1]) / dist,
					(source->position[2] - tm.position[2]) / dist
				};

				// Panning: dot product with listener's right vector
				// Positive = sound is to the right, negative = sound is to the left
				const float gain_panning = to_source[0] * tm.right[0] +
				                           to_source[1] * tm.right[1] +
				                           to_source[2] * tm.right[2];
...
void tm_init(tm_callbacks callbacks, int sample_rate) {
	...
	// Default listener orientation: facing -Z, right is +X
	tm.forward[0] = 0.0f; tm.forward[1] = 0.0f; tm.forward[2] = -1.0f;
	tm.right[0] = 1.0f; tm.right[1] = 0.0f; tm.right[2] = 0.0f;
...
void tm_update_listener(const float* position, const float* forward) {
	...
	float rx = -forward[2];
	float rz = forward[0];
	// Normalize the right vector (only x and z components, y is always 0)
