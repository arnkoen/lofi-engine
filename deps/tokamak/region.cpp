...
void neCoordList::Add(neRigidBodyBase * bb, neRigidBodyBase * hint, s32 hintCoord)
{
	...
	if (lentryItem->entity.value >= lentryItem->prev->entity.value)
	{
		//already in place
		return;
	}
	...
	if (!hint)
	{
		...
		do 
		{
			if (lentryItem->entity.value < cur->entity.value)
			{
				if (cur->prev)
				{
					cur = cur->prev;
		...
		do 
		{
			if (hentryItem->entity.value <= cur->entity.value)
			{
				ASSERT(hentryItem->entity.bb != cur->entity.bb); //should go pass the same body's low end
	...
	else
	{
		...
		for (s32 i = 0; i < 2; i++)
		{
			...
			if (i == 0)
			{
				...
				if (entryItem->entity.value < cur->entity.value)
					searchUp = true;
				else
					searchUp = false;
			}
...
void neCoordList::Sort(bool sortOnly)
{
	...
	while (coordList.used)
	{
		...
		if (insert->entity.flag == CCoordListEntry::LowEnd)
		{
			...
			{
				//compare
				if (cur->entity.bb == insert->entity.bb)
				{
					if (cur->prev)
					{
						cur = cur->prev;
				...
				{
					if (insert->entity.value < cur->entity.value)
					{
						// update overlap status
						if (cur->entity.flag == CCoordListEntry::HighEnd && !sortOnly)
						{
							neRigidBody_ * b1 = cur->entity.bb->AsRigidBody();

							neRigidBody_ * b2 = insert->entity.bb->AsRigidBody();

							if (b1 && b2)
							{
								if (! (b1->IsParticle() && b2->IsParticle()))
									region->ToggleOverlapStatus(cur->entity.bb, insert->entity.bb, dimPower2);
							}
							else
							{
								region->ToggleOverlapStatus(cur->entity.bb, insert->entity.bb, dimPower2);
		...
		else //HighEnd
		{
			...
			while (!done && cur)
			{
				...
				else
				{
					...
					{
						// update overlap status
						if (cur->entity.flag == CCoordListEntry::LowEnd && !sortOnly)
						{
							neRigidBody_ * b1 = cur->entity.bb->AsRigidBody();
