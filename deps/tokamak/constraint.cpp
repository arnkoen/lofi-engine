			...
			else
			{
				...
				else
				{
					...
					while (bodyHandle)
					{
						...
						bBodyHandle->entity->SetConstraintHeader(header);
					}
...
*/
	bodyAHandle.entity = this;

	bodyBHandle.entity = this;
...
void neConstraintHeader::TraverseApplyConstraint(neBool doCheckSleep)
{
	...
	while (bodyHandle)
	{
		if (!bodyHandle->entity->AsRigidBody())
		{
			bodyHandle = bodyHandle->next;
		...
		bodyHandle->entity->AsRigidBody()->needRecalc = false;

		neRigidBody_* rb = bodyHandle->entity->AsRigidBody();
/*
